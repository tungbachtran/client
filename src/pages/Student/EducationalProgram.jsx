        ...
        const fetchData = async () => {
            ...
            try {
                const { data: userInfor } = await axios.get(
                    `https://10.10.36.197:5001/api/user/${user.name}`
                )
                const educationalProgramId = userInfor.educationalProgram.educationalProgramId

                const { data: programData } = await axios.get(
                    `https://10.10.36.197:5001/api/education-program/${educationalProgramId}`
                )
                setProgram(programData)
                const { data: coursesData } = await axios.get(
                    `https://10.10.36.197:5001/api/education-program/course/${educationalProgramId}`
                )
                setCourses(
                    coursesData
