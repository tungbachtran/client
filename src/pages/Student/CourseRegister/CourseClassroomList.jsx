...
export default function CourseClassList({ setLoading, user }) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (courseClassId) => {
                return (
                    <Button type="primary" onClick={() => handleRegister(courseClassId)}>
                        Đăng ký
                    </Button>
                )
            }
    ...
    const handleRegister = async (courseClassId) => {
        try {
            await axios.post(
                `https://10.10.36.197:5001/api/course-classroom/user/${user.name}/${courseClassId}`
            ) //user.name = userId
            setModal({
                isShow: true,
            ...
            msg =
                err.response.data === 'Have Participated In This Course'
                    ? 'Đã đăng ký học phần này'
                    : msg
            setModal({
                isShow: true,
        ...
        const fetchData = async () => {
            ...
            try {
                const { data: course } = await axios.get(
                    `https://10.10.36.197:5001/api/course/${courseId}`
                )
                ...
                const { data: courseClasses } = await axios.get(
                    `https://10.10.36.197:5001/api/course-classroom/course/${courseId}`
                )
