...
function MessageStudentListModal({ messages, isVisible, onClose, info, classId }) {
                ...
                const messagesWithNames = await Promise.all(
                    messages.map(async (message) => {
                        const { data } = await axios.get(
                            `http://192.168.1.7:5148/api/user/${message.senderId}`
                        )
                        return {
                            ...message,
    ...
    useEffect(() => {
        const connect = new signalR.HubConnectionBuilder()
            .withUrl('http://192.168.1.7:5148/chatHub') // URL tới SignalR hub
    ...
    const handleSendMessage = async () => {
        if (newMessage.trim() && connection) {
            try {
                ...
                const { data } = await axios.get(`http://192.168.1.7:5148/api/user/${info}`)
