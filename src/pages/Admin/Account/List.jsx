...
export default function UserList({ setLoading }) {
    ...
    const handleSelectFaculty = (data) => {
        ...
        if (data.facultyId === 0) {
            let temp = []
            axios
                .get(`https://10.10.36.197:5001/api/classroom`)
                .then((res) => {
                    temp = res.data
        ...
        axios
            .get(`https://10.10.36.197:5001/faculty/classes/${data.facultyId}`)
            .then((res) => {
                temp = temp.concat(
                    res.data.map((_class) => {
                        return {
    ...
    const handleSelectClass = (data) => {
        ...
        if (data.id === 0) {
            axios
                .get(`https://10.10.36.197:5001/api/user/faculty/${data.faculty}`)
                .then((res) => {
                    let tempData = []
        ...
        axios
            .get(`https://10.10.36.197:5001/api/user/class/${data.id}`)
            .then((res) => {
                let tempData = []
    ...
    const handleDeleteUser = (id) => {
        axios
            .delete(`https://10.10.36.197:5001/api/user/${id}`)
            .then(() => {
