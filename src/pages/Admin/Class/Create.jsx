...
export default function CreateClass({ setLoading }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                let resFaculties = await axios.get('http://localhost:5148/api/faculty')
                setFaculties(resFaculties.data)
                let resPrograms = await axios.get('http://localhost:5148/api/education-program')
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(`http://localhost:5148/api/classroom`, data)
            setModal({
                isShow: true,
