...
export default function CreateClass({ setLoading }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true)
            try {
                let resFaculties = await axios.get('https://10.10.36.197:5001/api/faculty')
                setFaculties(resFaculties.data)
                let resPrograms = await axios.get('https://10.10.36.197:5001/api/education-program')
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(`https://10.10.36.197:5001/api/classroom`, data)
            setModal({
                isShow: true,
