...
export default function ClassList({ setLoading }) {
    ...
    const fetchClasses = async (data) => {
        ...
        if (data.facultyId === 0) {
            try {
                const { data: classesData } = await axios.get(`https://10.10.36.197:5001/api/classroom`)
                setClasses(
        ...
        try {
            const { data: classesData } = await axios.get(
                `https://10.10.36.197:5001/api/faculty/classes/${data.facultyId}`
            )
            setClasses(
                classesData.map((_class) => {
                    return {
    ...
    const handleDelete = async (id) => {
        setLoading(true)
        try {
            await axios.delete(`https://10.10.36.197:5001/api/classroom/${id}`)
