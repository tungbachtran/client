...
export default function CourseClassDocuments({ setLoading }) {
    ...
    const fetchTitleId = async () => {
        ...
        try {
            const response = await axios.get(
                `https://10.10.36.197:5001/api/course-classroom/${courseClassId}`
            )
    ...
    const fetchTitle = async () => {
        setLoading(true)
        try {
            const response = await axios.get(`https://10.10.36.197:5001/api/course/${titleId}`)
    ...
    const fetchDocuments = async () => {
        setLoading(true)
        try {
            const response = await axios.get(`https://10.10.36.197:5001/api/documents/${courseClassId}`)
    ...
    const handleAdd = async () => {
        ...
        try {
            await axios.post(
                `https://10.10.36.197:5001/api/documents/${courseClassId}?Content=${values.name}`,
                formData
            )
    ...
    const handleDelete = async (documentId) => {
        try {
            await axios.delete(`https://10.10.36.197:5001/api/documents/${documentId}`)
