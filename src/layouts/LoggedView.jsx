...
import FalcutyManagement from '../pages/Admin/Falcuty'
import Documents from './../pages/Teacher/Documents'
import CourseClassDocuments from './../pages/Teacher/CourseClassDocuments'
import CourseDocuments from './../pages/Student/CourseDocuments'
import MessageTeacher from './../pages/Teacher/Teacher-Message'
...
export default function LoggedView({ user, handleLogout }) {
    ...
    if (user.role === 'Student') {
        routesByRole = [
            ...
            <Route path={`/auth/course-document`} key={6}>
                <CourseDocuments setLoading={customSetLoading} user={user} />
            </Route>,
            <Route path={`/auth/message-student`} key={7}>
                <MessageStudent setLoading={customSetLoading} user={user} />
            </Route>,
            <Route path="*" key={5}>
    ...
    if (user.role === 'Admin') {
        routesByRole = [
            ...
            <Route path={`/auth/manage-falcuty`} key={6}>
                <FalcutyManagement setLoading={customSetLoading} />
            </Route>,
            <Route path="*" key={5}>
    ...
    if (user.role === 'Teacher') {
        routesByRole = [
            <Route path={`/auth/course-in-charge/:courseClassId`} key={0}>
                <CourseInChargeDetail user={user} setLoading={customSetLoading} />
            </Route>,
            <Route path={`/auth/course-in-charge`} key={1}>
            ...
            <Route path={`/auth/documents`} key={6}>
                <Documents setLoading={customSetLoading} user={user} />
            </Route>,
            <Route path={`/auth/course-class-documents/:courseClassId`} key={7}>
                <CourseClassDocuments user={user} setLoading={customSetLoading} />
            </Route>,
            <Route path={`/auth/message-teacher`} key={8}>
                <MessageTeacher user={user} setLoading={customSetLoading} />
            </Route>,
            <Route path="*" key={5}>
